...
def parse_args():
    ...
    parser.add_argument(
        "--config_yaml",
        type=str,
        default='models_yaml/anytext2_sd15.yaml',
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default='./anytext_v2.0_laion_generated/',
        help="output path"
    )
    parser.add_argument(
        "--json_path",
...
if __name__ == "__main__":
    ...
    for i in range(nproc):
        ...
        os.system('sleep 1')
        cmds += [f'export CUDA_VISIBLE_DEVICES={gpu_ids[i]}  && python {exec_path}  --input_json {tmp_file}  --output_dir {output_dir} --ckpt_path {ckpt_path} --config_yaml {config_yaml} && echo proc-{i} done!']
    cmds = ' & '.join(cmds)
    os.system(cmds)
    print('Done.')
    os.system('sleep 2')
